...
-(void)buttonClick:(UIButton *)sender{
    ...
    if([@"4.3" isEqual:code]){
        ...
        [SobotCallOpenApi queryLoginBingInfo:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
            if(code == CALL_CODE_SUCCEEDED){
        ...
        [SobotCallOpenApi queryUnusedExts:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
            if(code == CALL_CODE_SUCCEEDED){
                [[ZCGuideData getZCGuideData] showAlertTips:[NSString stringWithFormat:@"成功\n%@",msg]  vc:self blcok:^(int code) {
                    [self.navigationController popViewControllerAnimated:YES];
                }];
        ...
        [SobotCallOpenApi queryAgentBusyStatus:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
            if(code == CALL_CODE_SUCCEEDED){
                [[ZCGuideData getZCGuideData] showAlertTips:[NSString stringWithFormat:@"成功\n%@",msg]  vc:self blcok:^(int code) {
                    [self.navigationController popViewControllerAnimated:YES];
                }];
        ...
        NSString *str = [SobotCallOpenApi searchPlayCategory];
