...
@interface ZCConfigTMDetailController ()<UITableViewDelegate,UITableViewDataSource,ZCConfigBaseCellDelegate,ZCConfigCodeDelegate>{
...
@implementation ZCConfigTMDetailController
    ...
    if(sobotIsNull([ZCGuideData getZCGuideData].callCacheEntity)){
        ...
        [[ZCGuideData getZCGuideData] showAlertTips:tips vc:self blcok:^(int code) {
            ...
            if(sectionIndex == ZCSectionIndex31){
                // 初始化
                [SobotTMOpenApi initWithConfig:[ZCGuideData getZCGuideData].callCacheEntity result:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
                ...
                // 登录
                [SobotTMOpenApi loginWithAcount:[ZCGuideData getZCGuideData].loginAccount password:[ZCGuideData getZCGuideData].loginPassword token:[ZCGuideData getZCGuideData].token result:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
                    if(code == CALL_CODE_SUCCEEDED){
                ...
                // 置忙
                [SobotTMOpenApi noReady:[ZCGuideData getZCGuideData].apiParamter.reasonCode resultBlock:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
                    if(code == CALL_CODE_SUCCEEDED){
                        [[ZCGuideData getZCGuideData] showAlertTips:[NSString stringWithFormat:@"成功\n%@",msg] vc:self blcok:^(int code) {
                            [self.navigationController popViewControllerAnimated:YES];
                        }];
                ...
                [SobotTMOpenApi queryPhoneType:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
                    if(code == CALL_CODE_SUCCEEDED){
                        [[ZCGuideData getZCGuideData] showAlertTips:[NSString stringWithFormat:@"成功\n%@",msg] vc:self blcok:^(int code) {
                            [self.navigationController popViewControllerAnimated:YES];
                        }];
                ...
                // 延迟整理时长
                [SobotTMOpenApi delayACW:[ZCGuideData getZCGuideData].apiParamter.delayTime ResultBlock:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
                    if(code == CALL_CODE_SUCCEEDED){
                        [[ZCGuideData getZCGuideData] showAlertTips:[NSString stringWithFormat:@"成功\n%@",msg] vc:self blcok:^(int code) {
                            [self.navigationController popViewControllerAnimated:YES];
                        }];
                ...
                // 延迟整理时长
                [SobotTMOpenApi agentSendDtmf:[ZCGuideData getZCGuideData].apiParamter.dtmfDigits ResultBlock:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
                    if(code == CALL_CODE_SUCCEEDED){
                        [[ZCGuideData getZCGuideData] showAlertTips:[NSString stringWithFormat:@"成功\n%@",msg] vc:self blcok:^(int code) {
                            [self.navigationController popViewControllerAnimated:YES];
                        }];
                ...
                // 延迟整理时长
                [SobotTMOpenApi agentSetRouteExplicitRule:[ZCGuideData getZCGuideData].apiParamter.explicitRule explicitCode:[ZCGuideData getZCGuideData].apiParamter.explicitCode explicitNumber:[ZCGuideData getZCGuideData].apiParamter.explicitNumber resultBlock:^(NSInteger code, id  _Nullable obj, NSString * _Nullable msg) {
                    if(code == CALL_CODE_SUCCEEDED){
                        [[ZCGuideData getZCGuideData] showAlertTips:[NSString stringWithFormat:@"成功\n%@",msg] vc:self blcok:^(int code) {
                            [self.navigationController popViewControllerAnimated:YES];
                        }];
